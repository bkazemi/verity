...
OpenPGP is read by [OpenPGP.js](https://openpgpjs.org/), since the format's rules on when a valid signature still does not count are exactly what a partial reader gets wrong. The key has to be fit to sign **today**: not expired, not revoked, and carrying at least one user ID whose self-signature says so, which is also what gpg requires before it will import a key. The date a signature claims to have been made does not help an expired key, because the signer writes that date. SHA-1 is refused for every signature, including the key's own bindings and user ID certifications, so a key whose self-signatures are all SHA-1 cannot prove anything. A signing subkey counts only where the primary key bound it for signing and the subkey signed back, so nobody can staple their own subkey to a published key and sign as its holder. Nor can a published subkey be claimed under a key it never agreed to.
